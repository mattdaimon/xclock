...
画面の自動消灯を抑制する場合は、次のように変更します。

  set "KEEP_AWAKE=1"

Web版ではURLの末尾に `?wake=1` を付けます。

  https://mattdaimon.github.io/xclock/?wake=1

秒針と併用する場合は、2つのパラメータを `&` でつなぎます。

  https://mattdaimon.github.io/xclock/?seconds=1&wake=1

`wake=1` は、対応ブラウザへ画面の自動消灯を抑制するよう要求します。
手動ロックを解除する機能ではなく、省電力モード、ブラウザ、OS、端末状態によっては要求が拒否または解除される場合があります。
非対応または取得失敗の場合も、時計表示は通常どおり継続します。
...
5. 日本語などASCII以外の文字を含むユーザー名
   ...
   同梱のバッチファイルは、PowerShellを使用してWindowsのローカルパスを
   正規のfile URL形式へ変換します。

   日本語などASCII以外の文字を含むユーザー名やフォルダー名でも使用できます。
   コマンドへユーザー名を直接記述する必要はありません。

   PowerShellが組織のセキュリティポリシーで無効化されている環境では、
   xclock.batを使用できない場合があります。
